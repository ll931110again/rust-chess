...
use fetch::{build_client, enrich_articles, fetch_feed_items, FetchConfig};
use ingest::upsert_page;
use sources::news_sources;
use sqlx::SqlitePool;
...
static STATUS: std::sync::LazyLock<tokio::sync::RwLock<CrawlerStatus>> =
    std::sync::LazyLock::new(|| tokio::sync::RwLock::new(CrawlerStatus::default()));
...
pub fn spawn(pool: SqlitePool) {
    ...
    info!("Crawler scheduled every {}s", interval_secs);
}
...
pub async fn run_once(pool: &SqlitePool) -> anyhow::Result<()> {
    ...
    for source in sources {
        match fetch_feed_items(&client, &source, config.max_items_per_feed).await {
            Ok(items) => {
                ...
                let articles = enrich_articles(&client, items, &config).await;
                for article in articles {
                    match upsert_page(pool, &article).await {
                        Ok(true) => ingested += 1,
                        Ok(false) => {}
                        Err(e) => {
                            errors += 1;
                            warn!("Ingest failed for {}: {:#}", article.url, e);
                        }
                    }
                }
            }
    ...
    info!(
        "Crawler complete: {} feeds, {} seen, {} ingested, {} errors",
        source_count,
        seen,
        ingested,
        errors
